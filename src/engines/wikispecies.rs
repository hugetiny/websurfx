...
impl Wikispecies {
    ...
    fn build_url(&self, query: &str, page: u32) -> String {
        ...
        format!(
            "https://species.wikimedia.org/w/api.php?action=query&list=search&srsearch={}&format=json&sroffset={}&srlimit=20",
...
impl SearchEngine for Wikispecies {
    async fn results(
        &self,
        query: &str,
        page: u32,
            ...
            if let Some(search_results) = q.search {
                for result in search_results {
                    ...
                    if !title.is_empty() {
                        ...
                        let wiki_url = format!("https://species.wikimedia.org/wiki/{}", encoded_title);
                        results.push((
                            wiki_url.clone(),
                            SearchResult {
                                title,
                                ...
                                engine: vec!["wikispecies".to_string()],
                                relevance_score: 0.0,
                            },
                        ));
                    }
