...
#[derive(Deserialize)]
struct CodebergResponse {
    data: Option<Vec<CodebergRepo>>,
}
...
impl Codeberg {
    ...
    fn build_url(&self, query: &str, page: u32) -> String {
        ...
        format!(
            "https://codeberg.org/api/v1/repos/search?q={}&page={}&limit=20",
            encoded_query, page
        )
    }
...
impl SearchEngine for Codeberg {
    async fn results(
        ...
        let response: CodebergResponse = serde_json::from_slice(&bytes)
            .map_err(|_| Report::new(EngineError::UnexpectedError))?;

        let mut results = Vec::new();

        if let Some(repos) = response.data {
            for repo in repos {
                let name = repo.name.unwrap_or_default();
                let html_url = repo.html_url.unwrap_or_default();
