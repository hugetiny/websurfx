//! Wikicommons media search engine
...
impl Wikicommons {
    ...
    fn build_url(&self, query: &str, page: u32) -> String {
        ...
        format!(
            "https://commons.wikimedia.org/w/api.php?action=query&list=search&srsearch={}&format=json&sroffset={}&srlimit=20",
            ...
            if let Some(search_results) = q.search {
                for result in search_results {
                    ...
                    if !title.is_empty() {
                        ...
                        let wiki_url = format!("https://commons.wikimedia.org/wiki/{}", encoded_title);
                        results.push((
                            wiki_url.clone(),
                            SearchResult {
                                title,
