...
impl Wikiquote {
    ...
    fn build_url(&self, query: &str, page: u32) -> String {
        ...
        format!(
            "https://en.wikiquote.org/w/api.php?action=query&list=search&srsearch={}&format=json&sroffset={}&srlimit=20",
...
impl SearchEngine for Wikiquote {
    async fn results(
        &self,
        query: &str,
        page: u32,
            ...
            if let Some(search_results) = q.search {
                for result in search_results {
                    ...
                    if !title.is_empty() {
                        ...
                        let wiki_url = format!("https://en.wikiquote.org/wiki/{}", encoded_title);
                        results.push((
                            wiki_url.clone(),
                            SearchResult {
                                title,
                                ...
                                engine: vec!["wikiquote".to_string()],
                                relevance_score: 0.0,
                                ..Default::default()
                            },
                        ));
