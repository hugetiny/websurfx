...
impl Core {
    ...
    fn build_url(&self, query: &str, page: u32) -> String {
        ...
        format!(
            "https://api.core.ac.uk/v3/search/works?q={}&offset={}&limit=20",
...
impl SearchEngine for Core {
    async fn results(
        ...
        if let Some(papers) = response.results {
            for paper in papers {
                ...
                let year = paper.yearPublished.unwrap_or(0);
                let download_url = paper.downloadUrl;

                if !id.is_empty() && !title.is_empty() {
                    let paper_url = download_url.unwrap_or_else(||
